...
# --- BƯỚC 1: LOAD DỮ LIỆU & HUẤN LUYỆN RANDOM FOREST ---
@st.cache_resource
def train_forest_model():
    ...
    # Tạo Pipeline xử lý OneHotEncoding cho dữ liệu chữ và Random Forest
...
try:
    model, dataset, feature_names, col_hang, col_dong, col_nam, col_km, col_tt, col_pt, col_kv = train_forest_model()
    data_loaded = True
        ...
        with col_price:
            ...
            st.markdown(f"""
                <div style='background-color:#FFF3E0; border:2px dashed #FF5722; padding:40px; border-radius:12px; margin-top:15px;'>
                    ...
                    <p style='font-size: 14px; color: gray; margin-top: 15px;'>Giá ước tính dựa trên AI Random Forest và dữ liệu thị trường thực tế. Với mức giá này, xe sẽ dễ dàng thanh khoản trong khoảng 7-14 ngày.</p>
                </div>
            """, unsafe_allow_html=True)
